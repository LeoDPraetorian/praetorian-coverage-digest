...
for (const investigation of investigations) {
  for (const gap of investigation.control_gaps) {
    const consolidatedGap = {
      ...
      recommendation: investigation.recommendations.find(r =>
...
writeFile('phase-4/control-gaps.json', JSON.stringify({
  gaps: allGaps,
  metadata: {
...
```json
{
  "gaps": [
    ...
    },
    {
      ...
      "cvss_score": 7.8,
      "affected_components": [
        "modules/chariot/devops/cloudformation/iam-roles.yml"
      ],
