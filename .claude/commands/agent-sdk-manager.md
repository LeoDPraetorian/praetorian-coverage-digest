...
model: sonnet
allowed-tools: WebFetch, WebSearch, Bash, Write, Read, Task
---
...
**Example 1: TypeScript agent (minimal start)**

```
> /agent-sdk-write my-coding-agent

→ Asks: "TypeScript or Python?" → User: "TypeScript"
→ Asks: "What kind of agent?" → User: "Code review agent"
→ Asks: "Starting point?" → User: "Minimal hello world"
→ Checks latest SDK version: @anthropic-ai/claude-agent-sdk@1.2.0
→ Creates project structure with package.json (type: module)
→ Installs SDK and dependencies
→ Creates src/index.ts with basic query example
→ Runs: npx tsc --noEmit (type check passes)
→ Spawns: agent-sdk-verifier-ts for validation
→ Reports: Project ready, run with npm start
```

**Example 2: Python agent (full-featured)**

```
> /agent-sdk-write security-scanner

→ Language: Python
→ Type: Security review agent with custom tools
→ Starting point: Feature-rich example
→ Checks: claude-agent-sdk 1.5.0 (latest)
→ Creates project with poetry
→ Installs SDK + dependencies
→ Creates main.py with:
   - Custom security scanning tools via MCP
   - Streaming response handling
   - Proper error handling
→ Verifies: No syntax errors
→ Spawns: agent-sdk-verifier-py
→ Reports: Ready to run with python main.py
```

**Example 3: Agent with MCP integration**

```
> /agent-sdk-write data-analyst

→ Language: TypeScript
→ Type: Data analysis with database access
→ Starting point: MCP integration example
→ Checks SDK version and installs latest
→ Creates project with MCP server configuration
→ Adds .claude/ directory structure
→ Creates subagent for specialized tasks
→ Type checks pass, verifier confirms setup
→ Provides: Getting started guide with MCP setup
```
