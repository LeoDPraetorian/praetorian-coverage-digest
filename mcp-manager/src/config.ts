...
const CLAUDE_EXT_CONFIG_PATH = join(homedir(), ".mcp-manager.json");
...
export function readClaudeExtConfig(): ClaudeConfig {
	try {
		...
		console.error("Failed to read ~/.mcp-manager.json:", error);
		return {};
	}
}
...
export function writeClaudeExtConfig(config: ClaudeConfig): void {
	try {
		...
		console.error("Failed to write ~/.mcp-manager.json:", error);
		throw error;
	}
}
