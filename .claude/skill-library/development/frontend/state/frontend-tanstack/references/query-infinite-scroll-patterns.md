...
```typescript
function QueryBuilder() {
  const { data, refetch, resetCompletionGuard } = useQueryWithCompletion(
    (data) => {
...
const handleComplete = useCallback((data) => {
  ...
  setResults(data);
}, [filters, setResults]);  // Even with dependencies, can be stale
```
...
- [Zustand Integration](../../frontend-zustand-state-management/SKILL.md) - Store patterns
