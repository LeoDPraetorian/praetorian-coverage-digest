...
description: Use when testing APIs, microservices, validating API contracts, preventing breaking changes, or designing test strategies for distributed systems - comprehensive patterns for contract testing (Pact, Spring Cloud Contract), REST/GraphQL testing, integration testing with real dependencies, component testing with mocks, and performance/load testing (k6, Artillery, JMeter).
...
describe("Product CRUD", () => {
  ...
  it("UPDATE: modifies product", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body.price).toBe(12.0);
  });
...
describe("Filtering and sorting", () => {
  ...
  it("combines multiple filters", async () => {
    ...
    expect(response.body.items.every((p) => p.category === "electronics" && p.price >= 100)).toBe(
      true
    );
  });
});
...
await agent.loadTest({
  ...
  duration: "5min",
  scenarios: ["create-order", "concurrent-checkouts", "bulk-operations"],
});
