 ...
 */
function getScriptDirectory(importMetaUrl?: string): string {
  if (importMetaUrl) {
...
export function findProjectRoot(): string {
  ...
  throw new Error(
    'Could not find project root (no .claude directory found)\n' +
 ...
 * ```
 */
export function findProjectRootByMarker(marker: string): string {
 ...
 */
function findSkillInDirectory(dir: string, name: string): string | null {
  if (!existsSync(dir)) {
    return null;
  }
 ...
 */
function findAllSkillDirectories(dir: string, found: Set<string> = new Set()): string[] {
  if (!existsSync(dir)) {
...
export function resolveSkillDirectory(options: {
  ...
  if (options.skillName) {
    ...
    throw new Error(
      `Skill "${options.skillName}" not found in:\n` +
      `  - .claude/skills\n` +
