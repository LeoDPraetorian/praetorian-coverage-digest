# Phase 1 Integration - Business Context for Codebase Mapping
...
Phase 1 (Business Context Discovery) produces business-critical data that drives Phase 3 codebase mapping:
...
Without Phase 1 context, codebase mapping treats all components equally (security theater). With Phase 1 context, mapping focuses on protecting what matters most (risk management).
...
**`../phase-1/summary.md`** - Quick context overview (<2000 tokens)
...
**`../phase-1/data-classification.json`** - Crown jewels for prioritization
...
**`../phase-1/compliance-requirements.json`** - If mapping compliance-relevant components
...
## How Phase 1 Drives Codebase Mapping

| Phase 1 Input     | Impact on Mapping                                    | Example                                                                                                                       |
| ----------------- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
...
```bash
# Read crown jewels from Phase 1
CROWN_JEWELS=$(jq -r '.crown_jewels[].data_type' ../phase-1/data-classification.json)
...
**Scenario**: Phase 1 identifies crown jewels = `["payment_card_data", "user_credentials"]`
...
- `true` if component processes any crown jewel from Phase 1
- `false` otherwise
- Used to prioritize downstream Phase 4-6 analysis

**`crown_jewels_handled`** (array of strings):

- List of specific crown jewel data types this component handles
- Must match `data_type` values from Phase 1 `data-classification.json`
...
Add this section at the top of `summary.md` to connect Phase 3 to Phase 1 findings:
...
**Threat Actor Focus** (from Phase 1):
...
At the end of `summary.md`, include forward-looking guidance for Phase 4:

```markdown
## Recommended Focus Areas for Phase 4 (Security Controls Mapping)

Based on crown jewels and compliance requirements from Phase 1:
...
1. **PCI-DSS Requirement 3 Validation** (Critical Priority)
   ...
   - Phase 1 reference: PCI-DSS Level 1 requirement
...
2. **Authentication Control Review** (High Priority)
   ...
   - Phase 1 reference: user_credentials crown jewel
...
3. **Access Logging Validation** (High Priority - Compliance)
   ...
   - Phase 1 reference: SOC2 CC7.2, PCI-DSS Requirement 10
```
...
**Cannot proceed without Phase 1**. If Phase 1 files are missing:

```bash
# Check for Phase 1 summary
if [ ! -f "../phase-1/summary.md" ]; then
  ...
  echo ""
  echo "Phase 1 discovers WHAT you're protecting (crown jewels) and WHY (business impact)."
...
if ! jq -e '.crown_jewels | length > 0' ../phase-1/data-classification.json > /dev/null 2>&1; then
  echo "ERROR: data-classification.json missing crown_jewels array"
  echo "Phase 1 must identify at least one crown jewel for prioritization"
  exit 1
fi
```
...
4. Explains WHY Phase 1 is required (prioritization, not security theater)
...
- Brief overview of Phase 1 integration (3-5 sentences)
...
- `../phase-1/summary.md` - Business context from Phase 1
- `../phase-1/data-classification.json` - Crown jewels schema
- `../phase-1/compliance-requirements.json` - Regulatory requirements
