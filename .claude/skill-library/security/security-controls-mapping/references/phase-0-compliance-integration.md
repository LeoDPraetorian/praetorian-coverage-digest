# Phase 0 Integration - Compliance-Driven Control Evaluation
...
Phase 0 (Business Context Discovery) produces compliance requirements that drive Phase 2 security controls evaluation:
...
Without Phase 0 context, control mapping is generic (finds controls, but doesn't validate compliance). With Phase 0 context, mapping validates regulatory requirements (compliance-driven evaluation).
...
```bash
# Load compliance requirements from Phase 0
COMPLIANCE=$(jq -r '.applicable_regulations[].regulation' ../phase-0/compliance-requirements.json)
echo "Validating controls for: $COMPLIANCE"
```

**For each compliance requirement from Phase 0**:
...
**Cannot proceed without Phase 0 compliance requirements**:

```bash
# Check for Phase 0 compliance file
if [ ! -f "../phase-0/compliance-requirements.json" ]; then
  echo "ERROR: Phase 0 (Business Context Discovery) not complete."
  echo "Phase 2 requires compliance requirements to validate controls."
  echo ""
  echo "Run: skill: \"business-context-discovery\""
  echo ""
  echo "Phase 0 discovers WHICH regulations apply (SOC2, PCI-DSS, HIPAA, GDPR)."
...
if ! jq -e '.applicable_regulations | length > 0' ../phase-0/compliance-requirements.json > /dev/null 2>&1; then
  echo "ERROR: compliance-requirements.json missing applicable_regulations array"
  echo "Phase 0 must identify at least one regulation for validation"
  exit 1
fi
```
...
4. Explains WHY Phase 0 is required (compliance validation, not generic control inventory)
...
- `../phase-0/summary.md` - Compliance overview from Phase 0
- `../phase-0/compliance-requirements.json` - Detailed requirements with validation criteria
