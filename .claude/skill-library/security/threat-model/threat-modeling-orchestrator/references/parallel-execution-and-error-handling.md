...
for (const concern of criticalConcerns) {
  Task("security-controls-mapper", `Investigate ${concern.title}`, "security-controls-mapper");
}
...
3. Re-spawn codebase-sizer agent with reduced scope:
   ...
   Task(
     "codebase-sizer",
     `Assess ${reducedScope} (excluding node_modules, vendor)`,
     "codebase-sizer"
   );
   ```
...
**Cause**: Agent didn't use sizing-codebases skill, produced unstructured output
...
2. Re-spawn agent with explicit skill invocation requirement:
   ```typescript
   Task("codebase-sizer", "MUST use sizing-codebases skill for ${scope}", "codebase-sizer");
   ```
...
- **Phase 2 details**: [phase-2-sizing-codebases-workflow.md](phase-2-sizing-codebases-workflow.md)
