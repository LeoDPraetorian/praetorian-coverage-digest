...
### ⚠️ Known Issues with abortController

**Warning:** The router's `abortController.signal` has known bugs that may affect reliability:

- Signal may not fire properly in some navigation scenarios ([#1339](https://github.com/TanStack/router/issues/1339), [#3014](https://github.com/TanStack/router/issues/3014))
- `AbortError` can bubble up without a clear reason ([#4517](https://github.com/TanStack/router/issues/4517), [#3928](https://github.com/TanStack/router/issues/3928))
- Race conditions between navigation and abort signal dispatch

**Recommended workaround:** For critical abort handling, use TanStack Query's built-in signal instead:

```typescript
// ✅ RECOMMENDED: Use Query's signal for reliable abort handling
export const Route = createFileRoute("/posts")({
  loader: async ({ context }) => {
...
// ⚠️ USE WITH CAUTION: Router's abortController may be unreliable
export const Route = createFileRoute("/posts")({
  loader: async ({ abortController }) => {
    // May not abort reliably in all scenarios
    await fetch("/api/posts", { signal: abortController.signal });
  },
});
```
