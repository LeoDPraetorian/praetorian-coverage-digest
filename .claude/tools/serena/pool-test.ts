 ...
 */
import { serenaPool } from '../config/lib/serena-pool.js';
...
async function testPool() {
  ...
  try {
    const client1 = await serenaPool.acquire('chariot', env);
  ...
  try {
    const client2 = await serenaPool.acquire('chariot', env);
  ...
  try {
    const client3 = await serenaPool.acquire('praetorian-cli', env);
