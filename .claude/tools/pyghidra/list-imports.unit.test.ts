/**
 * Unit tests for list_imports wrapper
 *
 * Test Plan: 32 tests across 6 categories
 * _ Input Validation (8 tests)
 * _ MCP Integration (3 tests)
 * _ Response Filtering (5 tests) _ includes library field testing
...
/**
 * Create a mock import with defaults
 */
function createMockImport(overrides?: Partial<RawImport>): RawImport {
  return {
    name: 'printf',
    address: '0x00401000',
    type: 'function',
    library: 'libc.so.6', // Default library
...
/**
 * Create a mock MCP response with multiple imports
 */
function createMockMcpResponse(
  count: number,
  total: number,
  matching?: number
): RawListImportsResponse {
  return {
    imports: Array.from({ length: count }, (_, i) =>
      createMockImport({
        name: `import_func_${i}`,
...
describe('list_imports', () => {
  ...
  describe('3. Response Filtering Tests', () => {
    it('3.1: filters response to minimal schema (omits ordinal, internal_id)', async () => {
      ...
      const mockResponse: RawListImportsResponse = {
        imports: [
          {
            name: 'printf',
            address: '0x00401000',
    ...
    it('3.2: includes library field identifying import source module', async () => {
      const { listImports } = await import('./list-imports.js');
      ...
      // KEY TEST: Verify library field is present and correct
      expect(result.items[0].library).toBe('libc.so.6');
      expect(result.items[1].library).toBe('kernel32.dll');
    });

    it('3.3: includes accurate pagination metadata', async () => {
      const { listImports } = await import('./list-imports.js');
    ...
    it('3.4: achieves 97% token reduction through pagination', async () => {
      const { listImports } = await import('./list-imports.js');

      const imports = Array.from({ length: 25 }, (_, i) =>
        createMockImport({
          name: `import_function_${i}`,
  ...
  describe('4. Security Tests 🔴 CRITICAL', () => {
    ...
    it('4.2: rejects query with multiple greedy wildcards (ReDoS)', async () => {
      ...
      await expect(
        listImports.execute({
          binary_name: 'kernel32.dll',
          query: '.*.*.*strcpy', // Multiple greedy wildcards
          offset: 0,
          limit: 25,
        })
      ).rejects.toThrow(/unsafe.*regex.*pattern|redos/i);
    ...
    it('4.5: rejects query with control characters', async () => {
      ...
      await expect(
        listImports.execute({
          binary_name: 'kernel32.dll',
          query: 'strcpy\x00_init', // Null byte
          offset: 0,
          limit: 25,
        })
      ).rejects.toThrow(/control character/i);
  ...
  describe('5. Edge Cases', () => {
    it('5.1: returns empty array when no imports match query', async () => {
      const { listImports } = await import('./list-imports.js');
    ...
    it('5.2: returns empty array when binary has no imports', async () => {
      const { listImports } = await import('./list-imports.js');
