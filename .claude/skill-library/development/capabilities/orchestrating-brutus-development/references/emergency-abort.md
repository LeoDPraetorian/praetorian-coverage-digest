...
**Safe termination of Brutus plugin development workflows with state preservation.**
...
### Brutus-Specific Triggers

#### 6. Protocol Research Blocking

- Protocol auth strategy proves unviable
- Cannot obtain sufficient test coverage for error classification
- Library/dependency unavailable or incompatible

```json
{
  "abort_reason": "protocol_research_blocking",
  "abort_phase": "Phase 3: Codebase Discovery",
  "abort_trigger_detail": "No viable auth library found - protocol may require custom implementation"
}
```
...
```json
{
  ...
  "abort_trigger_detail": "Plugin does not implement brutus.Plugin interface correctly"
}
```

#### 8. Test Infrastructure Unavailable

- Go toolchain unavailable or misconfigured
- Required test dependencies missing
- Target service unreachable for integration testing

```json
{
  "abort_reason": "test_infrastructure_unavailable",
  "abort_phase": "Phase 13: Testing",
  "abort_trigger_detail": "Go modules unavailable - cannot run tests"
}
```
...
```json
{
  "abort_timestamp": "2026-01-27T15:30:00Z",
  "abort_reason": "protocol_research_blocking",
  "abort_phase": "Phase 3: Codebase Discovery",
  "abort_trigger_detail": "No viable auth library found - protocol may require custom implementation"
}
...
```yaml
workflow_id: "brutus-ssh-20260127"
...
plugin_files:
  - "{BRUTUS_ROOT}/internal/plugins/{protocol}/{protocol}.go"
  - "{BRUTUS_ROOT}/internal/plugins/init.go"
...
abort_info:
  ...
  can_resume: true
  resume_instructions: "Re-run with custom auth implementation approach"
```
...
echo "State preserved in MANIFEST.yaml. Resume with: /brutus resume {workflow_id}"
```
...
cp -r {worktree-dir}/{protocol}-brutus/.brutus-development/ \
      .claude/.output/archived/{protocol}-brutus-{timestamp}/
...
```bash
cd {worktree-dir}/{protocol}-brutus
git checkout -- internal/plugins/{protocol}/
git checkout -- internal/plugins/init.go
git clean -fd internal/plugins/{protocol}/
...
git worktree remove {worktree-dir}/{protocol}-brutus --force

# Remove OUTPUT_DIR and archives
rm -rf .claude/.output/{protocol}-brutus/
rm -rf .claude/.output/archived/{protocol}-brutus-*/
...
- Consider alternative auth library approach
- Verify Go toolchain availability before resuming

[If can_resume: true]
**Resume Command**: /brutus resume {workflow_id}
```
...
## Brutus-Specific abort_reason Values

| Reason                            | Phase | Description                              |
| --------------------------------- | ----- | ---------------------------------------- |
| `protocol_research_blocking`      | 3     | Cannot develop viable auth strategy      |
| `critical_compliance_finding`     | 10    | Plugin architecture fundamentally flawed |
| `test_infrastructure_unavailable` | 13    | Go toolchain/deps unavailable            |
...
1. User runs: `/brutus resume {workflow_id}`
...
- Verify Go toolchain availability before Phase 13
