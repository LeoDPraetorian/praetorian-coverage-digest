...
description: Use this agent when the user needs to interact with Chariot's API through the Praetorian CLI for tasks such as managing assets, running scans, retrieving security data, or performing administrative operations.\n\n<example>\n\nContext: User wants to list all assets in their organization.\n\nuser: 'Can you show me all the assets we have registered?'\n\nassistant: 'I'll use the praetorian-cli-expert agent to retrieve your asset list'\n\n<commentary>\n\nThe user is requesting data retrieval from Chariot's API, which requires the Praetorian CLI expertise.\n\n</commentary>\n\n</example>\n\n<example>\n\nContext: User wants to add a new domain to monitor.\n\nuser: 'Please add example.com to our monitored domains'\n\nassistant: 'I'll use the praetorian-cli-expert agent to add that domain and confirm it was added successfully'\n\n<commentary>\n\nThe user is requesting to modify data in Chariot, which requires Praetorian CLI operations.\n\n</commentary>\n\n</example>
type: analysis
permissionMode: plan
...
skills: 'mcp-tools-registry, debugging-systematically, verifying-before-completion'
model: opus
color: purple
---
