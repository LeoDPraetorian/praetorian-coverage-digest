...
/**
 * Find project root (handles super-repo and normal repo)
 */
function getRepoRoot(): string {
  const cwd = process.cwd();

  // Try to find .claude/skills/agent-manager (we're likely in scripts/src/)
  if (cwd.includes('.claude/skills/agent-manager')) {
    // Navigate up to repo root
    const parts = cwd.split('/.claude/');
    return parts[0];
  }

  // Fallback: assume we're already at root or can use git
  try {
