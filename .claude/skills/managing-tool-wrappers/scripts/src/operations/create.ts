...
export async function createWrapper(options: CLIOptions): Promise<number> {
  ...
  if (!fs.existsSync(tsconfigPath)) {
    ...
    const tsconfigTemplatePath = path.join(repoRoot, '.claude', 'skills', 'managing-tool-wrappers', 'templates', 'tsconfig.json.tmpl');
  ...
  const templatePath = path.join(repoRoot, '.claude', 'skills', 'managing-tool-wrappers', 'templates', 'unit-test.ts.tmpl');
  ...
  console.log('  .claude/skills/managing-tool-wrappers/references/new-workflow.md\n');
