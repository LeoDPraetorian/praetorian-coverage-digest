...
- Load from ~/.claude/.env (PAI standard)
 ...
 */
async function fetchByDate(date: string, options: { limit?: number } = {}): Promise<void> {
...
// Usage
throw new CLIError("Configuration file not found", "ERR_NO_CONFIG", "Run: mycli init");
...
async function readJsonFile<T>(path: string): Promise<T> {
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      throw new CLIError(`File not found: ${path}`, "ERR_FILE_NOT_FOUND", "Check the file path");
    }
...
describe("CLI", () => {
  ...
  it("handles errors gracefully", async () => {
    await expect(execAsync("./cli.ts invalid-command")).rejects.toThrow();
  });
