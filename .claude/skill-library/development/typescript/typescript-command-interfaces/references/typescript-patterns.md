...
export class PnpmError extends Error {
  ...
  constructor(code: string, message: string, opts?: { hint?: string; attempts?: number }) {
    super(message);
...
const config = (await loadConfig()).mapError((e) => `Config error: ${e}`).valueOrAbort();
```
...
```typescript
const [config, user, data] = await Promise.all([loadConfig(), fetchUser(), fetchData()]);
```
