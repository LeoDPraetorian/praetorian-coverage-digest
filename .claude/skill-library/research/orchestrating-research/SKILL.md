...
### Post-Research Compaction Gate

**BLOCKING GATE: Context Compaction Required**

Before proceeding to Phase 4 (Synthesis), you MUST:

1. **Invoke skill**: Read and follow `persisting-progress-across-sessions` Context Compaction Protocol
2. **Execute compaction**:
   - Write full research findings to output files (already done by agents in Phase 3)
   - Verify writes succeeded (check OUTPUT_DIR for agent output files)
   - Replace inline research content with summaries (< 200 tokens each)
   - Use file references, not inline content
3. **Verify compaction** (all must be true):
   - [ ] All research outputs written to files in OUTPUT_DIR
   - [ ] Context contains only summaries of agent findings, not full research
   - [ ] File references used instead of inline content for synthesis

**Cannot proceed to Phase 4 until all verification items checked.**

**If skipping compaction**: Use AskUserQuestion with explicit risk disclosure:
'Skipping compaction checkpoint. Risk: Context rot may degrade synthesis quality. Proceed anyway?'
...
| Skipping intent expansion for vague queries | Misses interpretations, narrow coverage        | Always run translating-intent in deep mode    |
...
| Skipping compaction gate                    | Context rot degrades synthesis quality         | Complete Post-Research Compaction Gate        |
...
9. **COMPACTION GATES ARE BLOCKING**: Research accumulates context rapidly. Do NOT proceed to synthesis without compacting research outputs to files. The Post-Research Compaction Gate between Phase 3 and Phase 4 is mandatory.
...
| `persisting-progress-across-sessions` | `skill: "persisting-progress-across-sessions"`                                   | Context compaction protocol (required at Post-Research Compaction Gate) |
