...
**Example:**
```typescript
function example(x: number) {  // +1 (function entry)
  if (x > 0) {                 // +1 (if)
    for (let i = 0; i < x; i++) {  // +1 (for loop)
      console.log(i);
    }
...
```typescript
if (a && b && c) {  // Complexity +3 (one for each &&)
  doSomething();
}
```
...
```typescript
if (a && b && c) {  // Complexity +1 (entire condition as one decision)
  doSomething();
}
```
...
} catch (e) {          // +1 (exception path)
  handleError(e);
} finally {            // +0 (always executes)
  cleanup();
}
...
```typescript
if (user && user.isActive && user.hasPermission) {  // +1 or +3?
  allow();
}
```
