...
const priorityFiles = investigations
  .flatMap(inv => inv.files_for_phase5)
  .filter(unique);
...
const testCase = {
  ...
  expected_behavior: investigation.recommendations
    .find(r => r.priority === 'immediate')
...
const topGaps = gaps
  .sort(bySeverityAndCVSS)
  .slice(0, 50); // Focus on top 50 gaps
