...
**Structure and workflow for `.claude/docs/agents/AGENT-LOOPHOLES.md` tracking file.**

---

## File Location

`.claude/docs/agents/AGENT-LOOPHOLES.md`
...
```bash
grep "| ❌" .claude/docs/agents/AGENT-LOOPHOLES.md
```

**Count by pattern:**

```bash
awk -F'|' '{print $5}' .claude/docs/agents/AGENT-LOOPHOLES.md | \
  sort | uniq -c | sort -rn
```
...
```bash
awk -F'|' '{print $3}' .claude/docs/agents/AGENT-LOOPHOLES.md | \
