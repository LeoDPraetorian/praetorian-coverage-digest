...
allowed-tools: Read, Bash, Grep, Glob, TodoWrite, Task, AskUserQuestion, Skill
---
...
Threat modeling generates extensive context. Do NOT proceed past compaction checkpoints without completing verification.
...
**For complete workflow details, see [Phase 2 Workflow](references/phase-2-sizing-codebases-workflow.md)**:
...
3. **Verify compaction** (all must be true):
   ...
   - [ ] Context contains only summary, not full details
...
'Skipping compaction checkpoint. Risk: Context rot may degrade performance in later phases. Proceed anyway?'
...
### Phase 4 → Phase 5 Transition: Compaction Gate
...
1. **Invoke skill**: Read and follow `persisting-progress-across-sessions` Context Compaction Protocol
2. **Execute compaction**:
   - Write full Phase 4 details to progress file
   - Verify write succeeded
   - Replace inline content with summary (< 200 tokens)
   - Use file references, not inline content
3. **Verify compaction** (all must be true):
   - [ ] Progress file updated with full Phase 4 output
   - [ ] Context contains only summary, not full details
   - [ ] File references used instead of inline content

**Cannot proceed to Phase 5 until all verification items checked.**
...
Task(
  ...
  `Generate prioritized security test plan from Phase 5 threats`,
  "security-test-planner"
);
```
...
- [references/phase-2-sizing-codebases-workflow.md](references/phase-2-sizing-codebases-workflow.md) - Codebase sizing details
...
- `sizing-codebases` - Phase 2 methodology (used by codebase-sizer agent)
- `mapping-codebases` - Phase 3 methodology (used by codebase-mapper agent)
...
- `persisting-progress-across-sessions` - Context compaction protocol (REQUIRED at compaction gates)
