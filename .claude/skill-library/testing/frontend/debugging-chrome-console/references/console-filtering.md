...
const { listConsoleMessages } =
  await import("./.claude/tools/chrome-devtools/list-console-messages.ts");
...
const issues = result.messages.filter((m) => m.type === "error" || m.type === "warning");
...
const realIssues = result.messages.filter((m) => {
  ...
  if (m.type === "warning") {
    ...
    const isExpected = expectedWarnings.some((pattern) => m.text.includes(pattern));
...
const filtered = result.messages.filter((m) => {
  ...
  const shouldIgnore = ignorePatterns.some((pattern) => pattern.test(m.text));
  return !shouldIgnore;
});
```
...
async function getActionableErrors() {
  ...
  const expectedWarnings = ["React DevTools", "Download the React DevTools", "version mismatch"];
  ...
  const ownCode = filtered.filter(
    (m) => m.source.includes("localhost") && !m.source.includes("node_modules")
  );

  // Step 4: Sort by priority
  const priorities = { error: 1, warning: 2 };
  const sorted = ownCode.sort((a, b) => priorities[a.type] - priorities[b.type]);
