...
description: Use when developing Chariot backend integrations with third-party APIs (Shodan, Wiz, Qualys, etc.) for asset discovery or vulnerability sync - orchestrates 8-phase workflow with specialized agents, human checkpoints, and P0 compliance validation (VMFilter, CheckAffiliation, ValidateCredentials, errgroup, pagination)
...
| 5     | P0 Compliance Verification             | 🛑 P0 violations (if any)                 | p0-compliance-review.md                            |
| 6     | Two-stage review                       | 🛑 Max retries exhausted                  | spec-compliance.md, quality-review.md, security.md |
| 7     | test-lead → backend-tester             | 🛑 Max retries exhausted                  | test-plan.md, tests, test-validation.md            |
| 8     | frontend-developer (conditional)       | Decision checkpoint (see criteria)        | Frontend files, frontend-integration-log.md        |
| 9     | Completion                             | Final verification, merge/PR/keep options | metadata.json updated to 'complete'                |
...
└─────────────────┬───────────────────────────────────────────────────────┘
                  ...
                  ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  Phase 5: P0 Compliance Verification                                  │
...
└─────────────────┬───────────────────────────────────────────────────────┘
                  ...
                  ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  Phase 6: Two-Stage Review (GATED)                                      │
...
│  If 'Needs UI: YES' → Proceed to Phase 8                                │
│  If 'Needs UI: NO' → Skip to Phase 9 (document skip in metadata.json)   │
...
### Phase 6: P0 Compliance Verification
...
**See**: [references/phase-5-p0-validation.md](references/phase-5-p0-validation.md)
...
### Phase 7: Review (Two-Stage Gated)
...
**See**: [references/phase-6-review.md](references/phase-6-review.md) | [references/prompts/reviewer-prompt.md](references/prompts/reviewer-prompt.md)
...
**See**: [references/phase-7-testing.md](references/phase-7-testing.md) | [references/prompts/test-lead-prompt.md](references/prompts/test-lead-prompt.md) | [references/prompts/tester-prompt.md](references/prompts/tester-prompt.md)
...
### Phase 9: Frontend Integration (CONDITIONAL)

**Decision Point**: After Phase 7 completes, evaluate these criteria:
...
2. If 'API key' or 'OAuth2 with user consent' → Phase 8 REQUIRED
3. If 'service account' or 'seed-based' → Phase 8 SKIPPED
4. If unclear → Ask user via AskUserQuestion

**Trigger**: Phase 8 runs if ANY of these are true:
...
**Skip Phase 8 if ALL of these are true**:
...
**See**: [references/phase-8-frontend-integration.md](references/phase-8-frontend-integration.md)

**Gate Checklist:**

- [ ] Read architecture.md 'Frontend Requirements' section (from Phase 3)
- [ ] Evaluated decision criteria against integration requirements
- [ ] IF Phase 8 needed: `frontend-developer` spawned
- [ ] IF Phase 8 needed: UI enum added to `types.ts`
- [ ] IF Phase 8 needed: Logos added (dark/light SVG)
- [ ] IF Phase 8 needed: `useIntegration.tsx` updated
- [ ] IF Phase 8 needed: `frontend-integration-log.md` created
- [ ] IF Phase 8 skipped: Skip reason documented in metadata.json
...
**See**: [references/phase-9-completion.md](references/phase-9-completion.md)
...
| `p0-validator-prompt.md`    | Phase 5     | (orchestrator)       | P0 compliance verification             |
| `reviewer-prompt.md`        | Phase 6       | backend-reviewer     | Spec compliance and quality review     |
| `security-prompt.md`        | Phase 6       | backend-security     | Security review                        |
| `test-lead-prompt.md`       | Phase 7.1/6.3 | test-lead            | Test planning and validation           |
| `tester-prompt.md`          | Phase 7.2     | backend-tester       | Test implementation with mocks         |
| `frontend-prompt.md`        | Phase 8       | frontend-developer   | UI integration (conditional)           |
...
When agents return blocked, use routing table to determine next steps. Common block reasons: architecture_decision (→ integration-lead), security_concern (→ backend-security), missing_api_docs (→ AskUserQuestion), p0_violation (→ integration-developer).

**See**: [references/blocked-status-handling.md](references/blocked-status-handling.md) for complete routing table, handling protocol, and examples.
...
Output directory: `.claude/.output/integrations/YYYYMMDD-HHMMSS-{vendor}/`

**Key files by phase**: design.md (1), skill-summary.md+discovery.md (2), architecture.md (3), implementation-log.md (4), p0-compliance-review.md (4.5), review files (5), test-plan.md+test-validation.md (6), frontend-integration-log.md (7).

**See**: [references/output-directory-structure.md](references/output-directory-structure.md) for complete directory structure, metadata.json schema, and file descriptions.

## Key Differences from Other Orchestration Skills

Integration dev differs from feature/capability dev: Phase 2 includes skill check, Phase 5 has P0 validation, Phase 8 is conditional (not always), P0 focuses on VMFilter/CheckAffiliation/pagination (not component size).

**See**: [references/comparison-with-other-orchestrations.md](references/comparison-with-other-orchestrations.md) for complete comparison matrix and when to use each orchestration type.

## Rationalization Prevention

Agents rationalize skipping steps ("CheckAffiliation can be a stub", "I'll add ValidateCredentials later", "P0 validation is redundant"). **Key principle**: If you detect rationalization phrases, STOP. Return to phase checklist. Never self-approve P0 exceptions.

**See**: [references/rationalization-prevention-integration.md](references/rationalization-prevention-integration.md) for complete rationalization table, detection patterns, override protocol, and evidence-based gates.
...
| `validating-integrations`          | Phase 5  | P0 compliance verification                        |
| `finishing-a-development-branch`   | Phase 9    | Final verification and merge/PR options           |
...
Integration development is complete when ALL criteria are verified: Phase completion (8/8), Skill artifacts (`integrating-with-{vendor}` exists), P0 compliance (7/7 requirements), Review status (all APPROVED), Test status (≥80% coverage, all pass), Final verification commands (build/test/vet/lint pass), Human approval.

**See**: [references/exit-criteria.md](references/exit-criteria.md) for complete checklist, quality metrics schema, verification commands, and incomplete criteria protocol.
