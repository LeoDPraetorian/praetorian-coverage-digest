...
Use this table to evaluate whether Phase 7 is needed:
...
After Phase 6 completes, before starting Phase 7:

1. Read architecture.md from Phase 3 output
2. Check 'Frontend Requirements' section
3. If 'Needs UI: YES' → Proceed to Phase 7
4. If 'Needs UI: NO' → Skip to Phase 8
...
**Location**: Between Phase 6 completion and Phase 7 start
...
if grep -q "## Frontend Requirements" <<< "$architecture_md"; then
  ...
  if [ "$needs_ui" = "YES" ]; then
    echo "Phase 7 REQUIRED - Proceeding to frontend integration"
    # Spawn frontend-developer agent
  elif [ "$needs_ui" = "NO" ]; then
    echo "Phase 7 SKIPPED - No UI configuration needed"
...
When Phase 7 runs, the `frontend-developer` agent creates:
...
When Phase 7 is SKIPPED, update `metadata.json`:
...
When Phase 7 RUNS, update `metadata.json`:
...
### Problem: Unclear if Phase 7 is needed
...
**Solution**:
- If integration appears in UI integration picker → Phase 7 REQUIRED (even if read-only)
- If integration is completely internal → Phase 7 SKIPPED

## Related References

- [Phase 3: Architecture](./phase-3-architecture.md) - Where Frontend Requirements are documented
- [Phase 8: Completion](./phase-8-completion.md) - Final verification includes frontend verification if Phase 7 ran
...
- [ ] Orchestrator proceeds directly to Phase 8
