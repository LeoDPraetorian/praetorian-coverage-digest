...
For EACH task in architecture plan:
...
Task(
  ...
  prompt: `
    You are implementing Task N from the architecture plan.

    TASK SPECIFICATION:
    {FULL TASK TEXT - paste directly from architecture.md, don't reference file}
...
Task(
  ...
  prompt: `[Use SPEC COMPLIANCE template from prompts/reviewer-prompt.md]
    ...
    ARCHITECTURE REQUIREMENTS FOR THIS TASK:
    {task specification from architecture.md}
...
| Developer repeatedly blocked | Review requirements, may need to revise architecture |
...
**Solution**: Use batch mode. Not all capabilities benefit from per-task mode.
