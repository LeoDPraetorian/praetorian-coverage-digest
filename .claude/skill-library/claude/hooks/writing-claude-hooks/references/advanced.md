...
if [ "$test_count" -eq 0 ]; then
  echo '{"ok": false, "reason": "No tests were run"}' >&2
  exit 2
fi
```
...
echo '{"ok": false, "reason": "Operation blocked and logged to Slack"}' >&2
exit 2
```
...
if [ -f "$rate_file" ]; then
  ...
  if [ "$current_minute" = "$last_minute" ]; then
    if [ "$count" -gt 10 ]; then
      echo '{"ok": false, "reason": "Rate limit exceeded"}' >&2
      exit 2
    fi
    count=$((count + 1))
  else
...
if echo "$content" | grep -qE "(api[_-]?key|password|secret|token).{0,20}['\"]?[A-Za-z0-9]{20,}"; then
  echo '{"ok": false, "reason": "Potential secret detected in content"}' >&2
  exit 2
fi
