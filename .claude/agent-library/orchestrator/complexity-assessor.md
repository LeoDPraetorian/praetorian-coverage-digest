...
description: Use this agent when you need to assess the complexity of a proposed implementation or feature to determine how much planning, resources, and risk mitigation is required. This agent should be called before starting significant development work to inform project planning decisions.\n\n<example>\n\nContext: User is planning to implement a new authentication system and needs to understand the complexity involved.\n\nuser: "We need to add OAuth2 integration with multiple providers to our platform"\n\nassistant: "I'll use the complexity-analyzer agent to assess the implementation complexity and determine the appropriate planning depth for this OAuth2 integration."\n\n</example>\n\n<example>\n\nContext: Team is considering refactoring the database layer and wants to understand the scope.\n\nuser: "Should we migrate from DynamoDB to Neo4j for our asset relationships?"\n\nassistant: "Let me analyze the complexity of this database migration using the complexity-analyzer agent to understand the architectural impact and planning requirements."\n\n</example>
type: analysis
permissionMode: plan
...
**MANDATORY: Use calibrating-time-estimates skill before providing any duration estimates**
...
- Use calibration factors from calibrating-time-estimates skill
...
**When complexity estimates are wrong:**

Use debugging-systematically skill for four-phase framework.

**Estimation debugging:**
1. Investigate (what was estimated? what actually happened?)
2. Analyze (what complexity missed? what overestimated?)
3. Test hypothesis (check similar features)
4. Refine (improve estimation model)

**Example - estimate too high:**
```typescript
// ❌ WRONG: "Just apply ÷12 calibration"
// ✅ CORRECT: "Estimated edge cases + security. Actual: just basic flow. Root: Overestimated complexity. Fix: Refine model"
```

**Red flag**: Applying formula without understanding error = STOP

**REQUIRED SKILL:** Use debugging-systematically for investigation
