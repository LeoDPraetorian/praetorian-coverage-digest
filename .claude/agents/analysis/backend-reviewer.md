...
skills: adhering-to-dry, adhering-to-yagni, analyzing-cyclomatic-complexity, calibrating-time-estimates, discovering-reusable-code, debugging-systematically, enforcing-evidence-based-analysis, gateway-backend, persisting-agent-outputs, semantic-code-operations, using-skills, using-todowrite, verifying-before-completion
model: sonnet
color: cyan
---
...
- Full implementation review? → `enforcing-evidence-based-analysis` (read source first) + `adhering-to-dry` + `using-todowrite` + `verifying-before-completion` + Read reviewing-backend-implementations skill + gateway task specific library skills
- PR review? → `enforcing-evidence-based-analysis` + `adhering-to-dry` + `adhering-to-yagni` + Read reviewing-backend-implementations skill + gateway task specific library skills
...
3. **Review patterns** - Quality guidance for Go code review
...
| `handoff.next_agent` | `"backend-developer"` (for fixes) |
