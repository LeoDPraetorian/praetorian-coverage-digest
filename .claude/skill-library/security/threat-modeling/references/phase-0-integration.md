...
**This reference explains how Phase 3 (Threat Modeling) integrates with Phase 0 (Business Context Discovery) to produce risk-based, business-relevant threat models.**

---

## Overview

Phase 0 provides business context that transforms threat modeling from generic technical analysis to risk-based security assessment aligned with PASTA methodology.
...
```bash
# Read threat actors from Phase 0
cat ../phase-0/threat-actors.json
```
...
```json
{
  ...
  "threat_actor_context": {
    ...
    "rationale": "Applied because Phase 0 identified financial cybercriminals as primary threat actor"
  }
}
```
...
### 3.1 Load Business Impact from Phase 0
...
```
Business Impact Score (from Phase 0):
...
Likelihood: 3 (High) - No parameterized queries found in Phase 2
...
Business Impact: 1 (Low) - $50K downtime from Phase 0
...
Crown Jewel Bonus = +2 if threat targets crown jewel from Phase 0
...
Crown jewel bonus: +2 (targets payment_card_data from Phase 0)
...
```json
{
  ...
  "business_context": {
    ...
    "business_impact_score": 4,
    "business_impact_financial": "$365M (from Phase 0 scenario: data_breach_1M_card_records)",
...
```markdown
# Phase 3 Summary: Threat Model (Business Context Integrated)

## Business Context Foundation (Phase 0)
...
- **X Critical threats** (priority 13+) identified - all target crown jewels from Phase 0
...
**Before handing off to Phase 4, verify Phase 0 integration**:

- [ ] Loaded threat-actors.json from Phase 0
- [ ] Loaded business-impact.json from Phase 0
- [ ] Loaded data-classification.json from Phase 0
...
- [ ] Top threats reference actual financial data from Phase 0

**Error if Phase 0 files missing**: Cannot run Phase 3 without Phase 0 completion.

---

## Section 7: Example Comparison

### Without Phase 0 (Generic Threat Model)
...
### With Phase 0 (Business-Driven Threat Model)
...
- Main skill: `../SKILL.md`
- Phase 0 skill: `.claude/skill-library/security/business-context-discovery/SKILL.md`
